...
let temp = {}
const ops = ['+', '-']
export class _GroupVerify extends plugin {
  constructor() {
    super({
      name: '[peach-plugin]入群验证',
      dsc: '重新验证和绕过验证',
      event: 'message.group',
      priority: 5,
      rule: [
...
Bot.on('notice.group.increase', async (e) => {
  logger.mark(`[进群验证]收到${e.user_id}的进群事件`)
...
async function verify(user_id, group_id, e) {
  ...
  logger.mark(`[进群验证]进行${user_id}的验证`)

  const { times, range, time, remindAtLastMinute } = Config.groupverify
  const operator = ops[lodash.random(0, 1)]

  let [m, n] = [lodash.random(range.min, range.max), lodash.random(range.min, range.max)]
  while (m == n) {
  ...
  logger.mark(`[进群验证]答案：${verifyCode}`)
