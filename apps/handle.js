...
export class _Handle extends plugin {
  constructor() {
    super({
      name: '[peach-plugin]请求处理',
      event: 'message',
      priority: 500,
      rule: [
        {
        ...
        {
          reg: '^#?(同意|拒绝|查看)(全部)?(加|入|进)?群申请(\\s*)(\\d+)?$',
          fnc: 'GroupAdd',
          event: 'message.group'
        },
        {
          reg: '^#?(同意|拒绝|查看)(全部)?群邀请(\\s*)(\\d+)?$',
          fnc: 'GroupInvite'
        },
        {
          reg: '^#?查看全部请求$',
  ...
  async Handle(e) {
    ...
    if (e.isGroup) {
      ...
      let userId = await redis.get(`groupAdd:${source.message_id}`)
  ...
  async Replys(e) {
    ...
    if (e.source) {
      ...
      e.message[0].text = e.message[0].text.replace(/#|回复/g, '').replace(/^\s+|\s+$/g, '')
    } else {
  ...
  async addFriend(e) {
    ...
    if (!(e.bot ?? Bot).gl.get(Number(group))) { return e.reply('❎ 群聊列表查不到这个群，人家做不到的啦>_<~') }
  ...
  async GroupAdd(e) {
    ...
    if (/全部/.test(e.msg)) {
      ...
      let qq = e.msg.replace(/#|(同意|拒绝)(加|入|进)?群申请/g, '').trim()
