...
Bot.on('request', async (e) => {
  ...
  switch (e.request_type) {
    case 'group':
      switch (e.sub_type) {
        case 'invite':
          if (!Config.getWhole.groupInviteRequest) return false
        ...
        case 'add':
          if (Config.groupAdd.openGroup.includes(e.group_id)) {
            ...
            await redis.set(`peach:groupAdd:${sendmsg.message_id}`, e.user_id, { EX: 3600 })
          }
      ...
      break
    case 'friend':
      if (!Config.getWhole.friendRequest) return false
      logger.mark('好友申请')
      msg = [
        segment.image(`https://q1.qlogo.cn/g?b=qq&s=100&nk=${e.user_id}`),
        '[通知 - 添加好友申请]\n',
