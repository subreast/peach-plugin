...
export default class {
  constructor(e) {
    this.e = e
    this.Bot = e.bot ?? Bot
    this.MuteTaskKey = 'peach-plugin:MuteTasks'
  }
   ...
   * @returns {Promise<Array<Object>|Array<Array<string>>>} 如果info为false，返回被禁言成员对象的简单数组；否则，返回被禁言成员信息的数组。
  ...
  async getMuteList(groupId, info = false) {
    ...
    if (lodash.isEmpty(mutelist)) throw Error('没有被禁言的人哦~')
    ...
    return mutelist.map(item => {
      ...
      return [
        ...
        `禁言剩余时间：${common.formatTime((time - (Date.now() / 1000)), 'default', true)}\n`,
  ...
  async releaseAllMute(groupId) {
    ...
    return Promise.all(mutelist.forEach(item => this.Bot.pickGroup(groupId - 0).muteMember(item, 0)))
  }


  /** 查看定时禁言任务 */
  getMuteTask() {
    let RegEx = /群定时(禁言|解禁)(\d+)/
    let taskList = lodash.cloneDeep(loader.task)
    let MuteList = taskList.filter(item => /群定时禁言\d+/.test(item.name))
    let noMuteList = taskList.filter(item => /群定时解禁\d+/.test(item.name))
 ...
 */
  ...
  async noactiveList(groupId, times = 1, unit = '月') {
    ...
    if (lodash.isEmpty(list)) throw Error(`暂时没有${times}${unit}没发言的淫哦╮( •́ω•̀ )╭`)
    return list
  }
  ...
  async getNeverSpeak(groupId) {
    ...
    if (lodash.isEmpty(list)) throw Error('本群暂无从未发言的人哦~')
    return list
  }
  ...
  */
  static async getRedisMuteTask() {
    return JSON.parse(await redis.get('peach:MuteTasks'))?.map(item => {
      return {
        cron: item.cron,
        name: `群定时${item.type ? '禁言' : '解禁'}${item.group}`,
        fnc: () => {
  ...
  async delMuteTask(group, type) {
    let redisTask = JSON.parse(await redis.get(this.MuteTaskKey)) || []
    loader.task = loader.task.filter(item => item.name !== `群定时${type ? '禁言' : '解禁'}${group}`)
